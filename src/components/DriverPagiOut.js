...
const DriverPagiOut = ({city}) => {
  ...
  React.useEffect(() => {
    myAxios({
      url: `/api/drivers/getAllDriversByCityAndCategoryWithPagination/city/${city.length==1?data.city:city}/category/8?pageNumber=${pageNo}&pageSize=5&sortBy=id&sortDir=asc`,
    }).then((res) => {
      ...
      <div>
        <div className="container">
          ...
          {(itemsVeh.content && itemsVeh.content.length!=0)  ? (
            ...
            <h1 className="textblink text-danger ">SORRY ! No Outstation Drivers Are Available In {city} </h1>
            </div>
              )}
          </div>
        </div>
