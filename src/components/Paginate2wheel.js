...
const Paginate2wheel = ({city}) => {
  ...
  React.useEffect(() => {
    myAxios({
      url: `/api/vehicles/getAllVehiclesByCityAndCategoryWithPagination/city/${city.length==1 ? usercity.data.city : city}/category/5?pageNumber=${pageNo}&pageSize=3&sortBy=id&sortDir=asc`,
    }).then((res) => {
        ...
        <div className="container">
          <div className="row m-2">
            {(itemsVeh.content && itemsVeh.content.length!=0)  ? (
              ...
              <h1 className="textblink text-danger ">SORRY ! No Two Wheelers Are Available In {city}</h1>
              </div>
            )}
          </div>
        </div>
