  ...
  useEffect(() => {
    axios
      .get(
        ...
        `http://192.168.29.130:5000/api/vehicles/getVehiclesByCityandCategory/city/${city.length==1 ? usercity.data.city : city}/category/6`
      )
      .then((response) => {
        ...
        <div className="container-fluid">
          ...
          {news.data.length > 1 ? (
            <Carousel breakPoints={breakPoints} showArrows={true}>
              ...
              <h1 className="textblink text-danger ">SORRY ! No Three Wheelers Are Available In Your {city}</h1>
              </div>
              )}
          </div>
        </div>
