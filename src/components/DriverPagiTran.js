...
const DriverPagiTran = ({city}) => {
  ...
  React.useEffect(() => {
    myAxios({
      url: `/api/drivers/getAllDriversByCityAndCategoryWithPagination/city/${city.length==1?data.city:city}/category/7?pageNumber=${pageNo}&pageSize=5&sortBy=id&sortDir=asc`,
    }).then((res) => {
      ...
      <div>
        <div className="container">
          ...
          {(itemsVeh.content && itemsVeh.content.length!=0)  ? (
            ...
            <h1 className="textblink text-danger ">SORRY ! No Transport Drivers Are Available In {city} </h1>
            </div>
              )}
          </div>
        </div>
