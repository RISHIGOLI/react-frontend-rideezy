...
navigate({pathname:`/show/${id}`})
  ...
  React.useEffect(() => {
    myAxios({
      url: `/api/vehicles/getAllVehiclesByCityAndCategoryWithPagination/city/${city.length==1 ? usercity.data.city : city}/category/6?pageNumber=${pageNo}&pageSize=9&sortBy=id&sortDir=asc`,
    }).then((res) => {
      ...
      <div>
        <div className="container">
          ...
          {(itemsVeh.content && itemsVeh.content.length!=0)  ? (
              ...
              <h1 className="textblink text-danger ">SORRY ! No Three Wheelers Are Available In {city}</h1>
              </div>
              )}
          </div>
        </div>
